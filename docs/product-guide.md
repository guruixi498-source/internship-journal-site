...
云同步：通过 Supabase 实现登录、云端保存、最后同步状态和忘记密码流程。用户在不同电脑打开同一网页，登录同一账号后即可同步查看和编辑数据。
...
第四步：开启云同步  
进入“云同步”，直接使用邮箱和密码注册或登录。项目已内置 Supabase 云端配置，普通用户无需手动填写 URL 或 Key。

第五步：跨设备使用  
在新电脑打开同一网页，登录同一账号，即可同步查看历史记录，并通过同步状态了解最近一次云端保存或拉取时间。
...
Cloud Sync: Uses Supabase authentication and database storage, with last-sync status and password reset support. Users can open the same website on another computer and sign in with the same account to continue editing.
...
Go to Cloud Sync and sign up or sign in with an email and password. The Supabase cloud configuration is built in, so regular users do not need to enter a URL or key manually.

Step 5: Use across devices  
On another computer, open the same website and sign in with the same account to load previous records. The sync status shows the latest cloud save or pull time.
