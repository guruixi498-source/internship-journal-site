...
- 云同步：基于 Supabase 实现跨设备同步、在线编辑、最后同步状态和忘记密码流程。
...
3. 忘记密码时，可在云同步页面发送重置密码邮件并设置新密码。
4. 第一次登录时，如果云端没有数据，网站会把当前浏览器数据上传成第一份云备份。
5. 换电脑后打开同一网页，登录同一账号会自动拉取云端数据，并显示最后同步时间。
6. 开发者如需更换 Supabase 项目，可在新项目 SQL Editor 中执行 `supabase-schema.sql`，再替换前端默认云端配置。
...
- Cloud Sync: Use Supabase Auth and Database for cross-device sync, online editing, last-sync status, and password reset.
...
3. If a user forgets the password, the Cloud Sync page can send a password reset email.
4. If no cloud data exists, the current browser data will be uploaded as the first cloud backup.
5. On another computer, open the same website and sign in with the same account to load cloud data and view the latest sync status.
6. Developers who want to switch Supabase projects can run `supabase-schema.sql` in the new project and replace the default frontend cloud configuration.
